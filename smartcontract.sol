...
struct rentInfo {
    ...
    function setCar(uint _carId, string memory _car_info, string memory _additional, uint _duration) public {
        cars[msg.sender] = carInfo({carid: _carId, car_info: _car_info,additional: _additional,
            duration: _duration, isRented: false, allowedToUse: true
        });
...
function setRenters(address payable _owner, address _customer, uint _carid) public {
...
function getRenters() public view returns (address owner, address customer, uint carid) {
    rentInfo storage r = renters[msg.sender];
    return (r.owner, r.customer, r.carid);
}
